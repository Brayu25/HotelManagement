...
export class AuthService {
  ...
  login(data: any): Observable<any> {
    let CREATE_URL = `${environment.baseUrlAdmin}/api/v1/auth/admin/signin`;
  ...
  verifyOTP(body: any): Observable<any> {
    ...
    return this.http.get<any>(OTP_URL, { headers: this.headers , params: body})
      .pipe(
        map((response) => {
 ...
 updatePassword(resetPasswordDetails: any): Observable<any> {
    const updatePasswordUrl = `${environment.baseUrlAdmin}/api/v1/auth/AutoPasswordResest{AUTO}`;
